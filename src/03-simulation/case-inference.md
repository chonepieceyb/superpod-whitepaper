...
前文已经给出了完整的方法链条：负载模型定义压力，资源模型定义供给边界，系统模型把两者转化为候选点，校准验证负责收紧误差，共演进闭环解释边界为何持续移动。但对白皮书而言，方法只有真正跑通一次，才具备说服力。本节因此并不是与前文并列的新增主题，而是作为整章的**方法论验证案例**，用一个真实的推理架构决策过程，证明这套方法既能解释问题，也能支持决策。

案例的意义并不局限于“某种架构更优”这一局部结论，更重要的是它展示了帕累托分析如何从抽象框架变成工程工具：先定义目标与约束，再在配置空间中描出边界，最后通过校准与敏感性分析，区分哪些收益是稳健的、哪些只是局部改善、哪些变化真正推动了边界外移。读者在这里看到的，不是一段独立的实验报告，而是前文方法链条的一次完整闭环。
...
这正是本章反复强调的工程现实：**不是所有点都值得同样精细地算，真正稀缺的资源是高成本验证能力，而不是参数本身。**
...
最重要的不是某一行具体数据，而是整条链路已经被验证：这套方法能够把“局部经验”提升为“边界判断”，先画出边界在哪里，再判断在边界上应当如何取点，最后用校准和敏感性分析确认结论是否稳健。对本章而言，这才是案例真正要证明的能力。
