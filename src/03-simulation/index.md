...
前两章已经分别回答了两个问题：超节点的系统能力边界为何会被打开，以及这些被打开的边界如何通过软件栈被兑现。但对超节点产业而言，这还不够。真正决定方案比较能否成立的，是第三个问题：**当不同路线都宣称自己更快、更省、更开放时，我们究竟用什么尺度判断它们处在怎样的系统边界上；进一步说，如何度量你在当前能力边界上的位置。**
...
在这套能力体系中，案例的价值并不是提供一组孤立结论，而是验证这套方法能否真正作用于现实决策。换句话说，案例不是附加说明，而是用真实约束检验“边界判断”是否成立的必要环节。
...
当上述六个环节成立之后，建模仿真的结果就不再只是几张曲线或若干配置点，而会沉淀为两类可以持续复用的判断对象。

第一类是**当前边界地图**。它回答的是：在统一负载集、统一指标口径和统一校准基线上，不同参考设计位于边界的什么位置、各自适用于怎样的负载画像，以及理论前沿与工程可达前沿之间还存在怎样的差距。有了这张地图，参考设计比较就不再是静态规格对照，而成为带条件、带风险、带适用边界的方案族位置关系。

第二类是**边界变化假设集**。它回答的是：在已校准的当前边界之上，光互联、`HBM4`、`Chiplet`、`MoE`、长上下文等变量会把前沿向什么方向推、多大程度上缓解现有瓶颈、又会在何处引入新的约束。只有把这些变化组织成带条件的假设集，而不是零散展望，面向未来演进的判断才能建立在量化基线之上。

也正因为如此，建模仿真真正给出的并不只是“性能预测”本身，而是两类可持续更新的量化对象：一类用于理解当前边界，一类用于判断边界将如何继续移动。

## 前沿不是静态的 {#pareto-frontier}

从行业演进看，更重要的一点在于：帕累托前沿不是静态资产，而是系统进步是否真实发生的直接度量。所谓“边界外推”，就是整条前沿在多个维度上向更优方向平移，它至少来自两个典型来源。
...
这种逻辑已经在[方法论验证案例：异构推理架构的帕累托分析](case-inference.md)中得到具体展开。该案例在 `L40S + A100` 的异构 `PD` 分离架构上，通过 4 种候选架构、3 类请求场景以及多并发、多配置的系统级扫描，描出了 `tokens/s/gpu` 与 `tokens/s/user` 坐标系下的帕累托前沿，再结合安全边际校准和成本敏感性分析，区分出哪些收益是稳健的、哪些只是局部改善、哪些变化真正把边界向外推移。下表概括了几类典型优化路径对前沿的影响：
...
从更大的行业视角看，随着超节点从“把更多芯片连在一起”演进为“持续重写系统能力边界”的基础设施形态，建模仿真正在从辅助分析工具上升为一项基础能力：它不仅回答边界究竟在哪里、如何比较，也回答边界将如何继续移动。也只有当建模仿真能够把**当前边界地图**和**边界变化假设集**这两类对象稳定沉淀出来，行业对参考设计的比较、对未来变量的判断，才不必在每一轮技术跃迁中重新发明一套口径。
