...
前两章分别回答了"帕累托前沿如何被推移"（架构分析）和"前沿如何被兑现"（软件系统）。但一个更基本的问题尚未回答：**如何度量你在帕累托空间中的当前位置？** 当系统设计涉及算力、带宽、功耗、时延、成本等多个彼此耦合的维度时，纯经验判断无法区分"沿前沿移动"与"真正推移前沿"，更无法量化前沿外推的幅度与方向。

这正是建模仿真在本白皮书中的定位：**度量帕累托前沿的决策工具**。当系统规模进入万卡级，且负载形态从"规则稠密训练"演进到"稀疏/MoE、长序列、多模态与在线推理混合运行"时，试错成本过高——一次硬件选型误判、一次并行策略或调度参数配置错误，都可能带来数周进度损失与巨额算力浪费。仿真用一套可复用的规约、模型与推演框架，把"争论"转化为"可验证的假设"，把"经验"转化为"可校准的资产"。
...
下一章将把这些方法论真正沉淀为参考设计，回答"在当前帕累托前沿上，针对不同约束与负载画像，应该选择哪些点"。
