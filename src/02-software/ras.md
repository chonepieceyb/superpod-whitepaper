...
**Checkpoint /Restore 在 AI 系统 RAS 中的作用。** `Checkpoint / Restore`（`C/R`）是一种提升系统可靠性（Reliability）、可用性（Availability）与可维护性（Serviceability，`RAS`）的重要技术手段。其基本思想是在程序运行过程中周期性地保存执行状态，当系统发生故障时，可以从最近一次保存的状态恢复执行，从而避免从头开始重新计算。在 `AI` 计算基础设施中，`C/R` 已经成为支撑训练与推理系统稳定运行的重要机制。真正的意义不在于“系统多了一项恢复功能”，而在于规模上来之后，系统终于需要为“中断之后如何继续”建立一条制度化路径：
...
现有实践中，一个最重要的经验是：排障能力只有真正回到**任务视角**，才可能具备稳定的解释力和可操作性。相比于简单新增一层告警，更关键的是将不同层级的数据源和检查手段组织成一条**系统化的排障链路**，使平台能够围绕任务对象完成故障定界、故障定位和经验沉淀。
...
另一个关键经验是，平台能力必须逐步从事后排查演进到**在线检测与自动触发诊断**。相较于等待用户反馈或任务彻底失败之后再介入，更有效的方式是在任务运行过程中持续感知异常，并在识别到高风险信号之后自动触发后续诊断动作。
...
* **观测能力需要向底层运行时与通信层持续下钻**

在实际排障过程中，仅依赖任务日志和硬件告警，往往无法解释大量“没有明确报错，但任务确实异常”的问题。一个重要经验是，平台观测能力必须能够在必要时继续**下钻到通信事件、运行时行为以及设备侧细节**，才能对复杂问题形成更有解释力的判断。
...
从既有实践中还可以看到，许多线上高频问题都具有较强的复现模式。通信挂起、异常退出、设备报错、驱动与环境不匹配、混部兼容性问题等，往往并不是完全随机发生，而是会在不同时间、不同任务中反复出现。真正有价值的经验沉淀，不仅是记录问题现象本身，更包括**候选根因、现场追溯能力、临时缓解方案以及长期修复动作**。
...
所有观测、告警、诊断和恢复动作，最终都应映射到任务对象本身，能够明确回答：哪一个任务受到了影响，影响范围有多大，当前任务是否还能继续运行，以及平台建议采取什么恢复动作。

* **以分层诊断替代单点告警**

原子告警仍然重要，但它只能被视为诊断链路中的一个输入，而不应等同于诊断本身。未来系统需要同时具备规则排障、主动探测、在线诊断、案例检索与趋势分析等多层能力，才能形成真正可用的 `RAS` 体系。

* **以自动化方式沉淀人工经验**

排障路径、判断规则、典型案例以及缓解动作都应逐步模块化，而不应长期依赖少数个体经验。只有这样，系统才能在规模增长的过程中保持稳定的可服务能力。

* **以关键指标驱动系统建设**

后续系统模块设计和阶段目标拆解，应围绕若干核心指标展开，包括：
...
* **训练平台与推理平台共用方法论、分化实现形态**

训练与推理在外部表现和恢复策略上存在差异：训练更强调长周期稳定性、`checkpoint` 保护与恢复效率，推理更强调服务连续性、时延抖动控制和快速隔离。但从方法论上看，两者底层是一致的：都需要从任务视角组织对象关系、建立异常时间线、关联多源数据，并基于影响范围与恢复代价选择处理动作。因此，未来平台可以在统一方法论之下，针对训练和推理分别设计不同的实现形态。
