...
!!! abstract "本节定位 · 统一内存子系统 ②"
    本章"统一内存子系统"分三节递进：通用语义框架 → **NVIDIA 实现剖面** → 内存池化。三节共同回答一个总问题：**统一内存子系统要解决的不是"能不能访问远端显存"，而是"能否以可接受的一致性代价、地址复杂度和调度开销，把跨设备内存稳定兑现为 Goodput"。** 上一节建立了 Scale-Up 域内存语义的通用分析框架：四类操作、三类乱序、六个关键机制、两大协议路线。本节回答这个总问题中的第二个子问题：**这些能力如何被一个真实系统实例化？** 因此它不再讨论通用语义定义，而是以 NVIDIA NVLink/NVSwitch Fabric 为例，说明这些通用概念如何在具体系统中落地——GMMU 如何承接地址翻译、Fabric Address 如何实现统一寻址、Fabric Manager 与 IMEX 如何完成跨节点协调、Export/Import 如何把硬件能力暴露给软件。

**适用范围**：支持 NVLink/NVSwitch 的 NVIDIA GPU（Volta 及更新架构）；DGX、HGX、NVL72 等多 GPU 环境。
...
本节回答的是统一内存子系统的第二个子问题：**这些能力如何被一个真实系统实例化？** NVIDIA 给出的答案并不是发明一套全新抽象，而是把上一节提出的必要条件，拆解为一组可组合的工程模块：地址由 GMMU/PTE 承接，路由由 Fabric Base Address 与 NVSwitch 完成，协调由 Fabric Manager 与 IMEX 分层处理，编程接口则通过 Export / Import / Map 暴露给软件。
...
从"统一内存子系统"三节递进的视角看，本节证明了第 ① 节的通用语义框架并非抽象概念——它可以被拆解为 GMMU 页表配置、Fabric 地址编码、协调守护进程与分层软件接口等可实现的工程模块。下一节"内存池化"将把视角拉升到系统软件层，讨论如何在多层级异构存储间高效调度，把统一寻址能力转化为实际的 Goodput 提升。
