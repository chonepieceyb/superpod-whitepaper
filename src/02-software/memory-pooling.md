...
本节与前两节共同构成"统一访存"，三者在**不同抽象层次上递进互补**：
...
三节构成**定义能力 → 落地能力 → 调度能力**的完整链路：通用框架回答"什么是对的"，实现剖面展示"怎么做到"，内存池化解决"如何用好"。它们共同勾勒出超节点统一访存的可达能力边界。

## 本节收束：如何把能力变成收益而不是负担

本节回答的是统一访存的第三个子问题：**有了这些能力之后，系统软件如何把它们变成收益而不是负担？** 结论是，统一寻址和远端可访问性只是池化成立的前提，不是池化自动产生价值的保证。真正决定 Goodput 的，是系统软件能否根据数据热度、拓扑位置、容量层次和负载画像，做出正确的放置、迁移、复用与隔离决策。
