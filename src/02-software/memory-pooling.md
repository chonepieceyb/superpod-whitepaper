...
!!! abstract "本节定位 · 统一内存子系统 ③"
    本章"统一内存子系统"分三节递进：通用语义框架 → NVIDIA 实现剖面 → **内存池化**。三节共同回答一个总问题：**统一内存子系统要解决的不是"能不能访问远端显存"，而是"能否以可接受的一致性代价、地址复杂度和调度开销，把跨设备内存稳定兑现为 Goodput"。** 前两节回答了"硬件如何让远程内存可访问"；本节回答这个总问题中的第三个子问题：**有了这些能力之后，系统软件如何把它们变成收益而不是负担？** 因此它把视角拉升到系统软件层，讨论如何让异构内存被高效使用。
...
本节要回答的是更上层的问题：**"如何高效地管理和调度这些分散在多层级、多节点上的异构存储资源？"** ①② 提供的内存语义和统一寻址是池化的*前提*——没有硬件级的 Load/Store 语义和全局地址空间，池化调度器无法把远端 HBM 当作可调度资源；但光有"可访问"还不够，还需要系统软件在多层级异构存储之间做出智能的放置、迁移和回收决策。统一内存系统之所以能成为软件章的第一个例子，正因为它最直接地体现了这一点：**硬件打开了跨设备内存的可达性，软件决定这种可达性能否稳定兑现为 Goodput。**
...
这里的代价也必须被正视。容量扩展从来不是免费的：把更多 HBM、DDR、CXL 内存和 SSD 纳入统一池，会同时引入迁移开销、命中率风险、碎片管理、拓扑感知调度和一致性边界问题。如果这些额外成本失控，那么统一内存并不会提升 Goodput，反而会把系统拖入"看起来容量更大、实际上更难用"的陷阱。池化真正成立的条件，是**扩展出来的容量价值，持续大于新增的迁移与调度成本。**
