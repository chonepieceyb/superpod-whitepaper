...
!!! abstract "本节定位 · 统一内存子系统 ③"
    本章"统一内存子系统"分三节递进：通用语义框架 → NVIDIA 实现剖面 → **内存池化**。前两节回答了"硬件如何让远程内存可访问"；本节把视角拉升到系统软件层，回答"如何让异构内存被高效使用"。
...
本节要回答的是更上层的问题：**"如何高效地管理和调度这些分散在多层级、多节点上的异构存储资源？"** ①② 提供的内存语义和统一寻址是池化的*前提*——没有硬件级的 Load/Store 语义和全局地址空间，池化调度器无法把远端 HBM 当作可调度资源；但光有"可访问"还不够，还需要系统软件在多层级异构存储之间做出智能的放置、迁移和回收决策。
...
| **帕累托坐标** | 一致性模型 vs 性能 vs 复杂度 | 延迟 vs 带宽 vs 一致性开销 | 容量 vs 成本 vs 迁移开销 vs 命中率 |

三节构成**定义能力 → 落地能力 → 调度能力**的完整链路：通用框架回答"什么是对的"，实现剖面展示"怎么做到"，内存池化解决"如何用好"。它们共同定义了超节点统一内存子系统的帕累托前沿。
