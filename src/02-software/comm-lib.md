...
在超节点中，这个问题比传统集群更尖锐。数据并行、张量并行、流水并行和专家并行会同时存在，集合通信与单边通信也不再是互斥关系，而是分别服务于**规整高吞吐数据流**与**不规整细粒度数据流**。因此，本节沿着“集合通信收益”和“PGAS/SHMEM/单边通信”两条线索展开，并尽可能按原始编号结构组织内容。前者回答统一内存语义如何被进一步抽象为高层原语，后者回答这些语义如何更直接地暴露到运行时与编程模型中。
