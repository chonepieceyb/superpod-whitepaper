...
前三章建立了一套完整的分析链：架构分析定义了帕累托前沿的边界与外推机制，软件系统阐明了前沿如何被兑现为 Goodput，建模仿真提供了度量帕累托位置的方法论工具。本章的任务是把这些分析收束为一个具体的工程问题：**在当前帕累托前沿上，针对不同的约束优先级和负载画像，应该选择哪些点？**

需要强调的是，参考设计不是单点最优答案。帕累托前沿的定义本身就排除了"脱离约束的唯一最优解"——前沿上的每一个点，在至少一个维度上优于所有其他点，同时在另一些维度上被其他点超越。这正是帕累托最优的本质：**不存在一个方案能在所有维度上同时支配其他方案**。每一个参考设计都是在规模、时延、带宽、功耗、成熟度与软件复杂度的权衡空间中，针对特定约束优先级做出的具体工程选择。

## 五个设计，一条前沿

本章给出的五个参考设计，对应帕累托前沿上五个不同位置。它们之间不是"好与更好"的替代关系，而是"不同约束优先级下的不可支配点"：
...
没有任何一个方案在所有维度上胜过其余方案——这正是它们共同构成帕累托前沿、而非简单排序的原因。
...
这张表的核心信息是：**没有一列全是五星**。每个方案在某些维度上领先，在另一些维度上让出空间。读者应根据自身的约束优先级——当前阶段最受限的是生态兼容还是延迟？是规模还是可运维性？——在这条前沿上找到最匹配的位置。

上表展示的是参考设计的结构性定位。对于具体产品在这一坐标系上的实测数据和独立评分，请参阅 [SuperPod Pareto Index（SPI）](../07-spi.md)——厂商在 SPI 中提交产品规格与实测证据，编写委员会依据同一套七维度框架进行独立评估。
