...
- **标准总线方案（以 `UALink` 为代表）**在访存延迟和内存语义完整性维度上占据前沿，为此在生态成熟度和供应链就绪度上承担风险。
...
|:-----|:--------|:---------|:-------------|:----------|:-----------|
| **单跳/访存延迟** | ★★★★（<1 μs RTT） | ★★（250–500 ns） | ★★★（低直径） | ★★★★（直连光路） | ★★★★★（无包交换） |
| **规模上限** | ★★★（≤1024） | ★★★（≤1024） | ★★★★★（万卡级） | ★★★★（千–万卡） | ★★★（探索中） |
| **拓扑弹性** | ★（固定全交叉） | ★（固定 Spine-Leaf） | ★★★（组间 OCS） | ★★★★★（全拓扑重构） | ★★★★★（收发器级重构） |
| **生态成熟度** | ★★（交换芯片未量产） | ★★★★★（以太网生态） | ★★★（组件成熟，集成新） | ★★★（Google 验证） | ★（研究阶段） |
| **功耗与 TCO** | ★★★（电交换为主） | ★★★（电交换为主） | ★★★★（减少电交换层级） | ★★★★★（无源光层核心） | ★★★★★（潜力最优） |
| **软件复杂度**（★越多越简单） | ★★★（FAM 管理） | ★★★★★（复用以太运维） | ★★★（拓扑管理+OCS控制） | ★★（全栈协同设计） | ★（全新软件栈） |
| **适用负载** | TP-heavy/显存共享 | EP/DP/MoE/混部 | 大规模训练/混合 | 邻近通信/高容错 | 前瞻探索 |
...
这三个负载画像分别对应了超节点最典型的三类压力场景。W1 与 W2 侧重训练，分别考验对称通信（Dense TP）和非对称通信（MoE EP）下的互联性能；W3 侧重推理，考验延迟敏感场景下的内存弹性。在 Blackwell NVL72 的实践中，W2 场景产生了超过 2700 种并行策略组合（详见[第三章](../03-simulation/index.md#pareto-frontier)），充分说明了在统一负载画像下进行帕累托分析的必要性。
...
```vegalite
{
  ...
  "data": [
    {
      "name": "table",
      "values": [
        {"key": "延迟", "value": 4, "category": "标准总线 (UALink等)", "idx": 0},
        {"key": "规模", "value": 3, "category": "标准总线 (UALink等)", "idx": 1},
        {"key": "弹性", "value": 1, "category": "标准总线 (UALink等)", "idx": 2},
        {"key": "成熟度", "value": 2, "category": "标准总线 (UALink等)", "idx": 3},
        {"key": "TCO", "value": 3, "category": "标准总线 (UALink等)", "idx": 4},
        {"key": "软件简易度", "value": 3, "category": "标准总线 (UALink等)", "idx": 5},
  ...
  "scales": [
    {"name": "r", "type": "linear", "domain": [0, 5], "range": [0, 72], "zero": true},
    {
      ...
      "domain": ["标准总线 (UALink等)", "标准以太网 (ESUN)", "Dragonfly + OCS", "Torus + OCS", "Ring + dOCS"],
...
- **标准构型**：以当前更成熟的工程路径为核心，强调可落地性、生态兼容与可运维性。在帕累托前沿上，它们分别锚定两种最典型的实现方向：总线语义优先，以及以太生态优先。
- **探索构型**：面向更大规模、更低时延或更高带宽目标，在拓扑与光交换等方向上主动引入更强的系统重构能力。在帕累托前沿上，它们的位置更偏向"规模–弹性–能效"端。

### 标准构型

- [标准总线方案](standard-bus.md)：以总线/内存语义为核心，以 `UALink` 等路线为代表，在帕累托前沿上占据**访存延迟与内存语义完整性的极值位置**。
- [标准以太网方案](standard-ether.md)：以开放以太网生态为基础，在总线语义目标之上融合实现内存语义，在帕累托前沿上占据**生态开放性与可运维性的极值位置**。
