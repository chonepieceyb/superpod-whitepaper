...
- **标准总线方案（`UALink` 为代表）**把强语义访问看作第一约束，核心目标是把更多远端资源继续组织成近似本地资源。它最接近前两章里“统一资源、强语义访问、`Goodput` 兑现”的主问题。
- **标准以太网方案（`ESUN/SUE/CLink` 等）**优先守住生态连续性、供应链连续性与运维连续性，在此基础上接受部分语义和时延让步，换取现实世界里更强的可交付性。
