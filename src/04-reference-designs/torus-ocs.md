...
`OCS + Torus` 突破了传统电交换网络的层级和端口数约束。通过网络扁平化和物理解耦，新增算力单元接入 `OCS` 后即可被动态纳入某个 `Torus` 切片，从而沿着“更大超节点”方向平滑演进。

在谷歌公布的公开数据中，`TPU v4` 的 `3D Torus` 超节点网络成本占系统总成本的比例低于 `5%`，网络功耗占系统总功耗的比例低于 `3%`，显著低于典型 `Clos` 网络常见的 `10%` 到 `20%` 区间。以端侧每卡出 `6` 个端口为例，`Clos` 架构每卡至少需要 `12` 个光模块，而 `OCS + 3D Torus` 平均每卡约 `1.5` 个光模块，硬件数量和运维压力都显著下降。
