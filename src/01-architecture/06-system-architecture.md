...
如前文所述，PCIe 总线访存体系的核心挑战在于如何突破单一 PCIe Root Complex（RC）的物理边界，实现跨节点的统一地址空间。闭源生态中，NVSwitch 提供了一种专有解法；而在开放标准阵营，PCIe 及其承载的 CXL 正通过物理层与链路层的重构，逐步从通用外设通道演进为支撑超节点 Scale-Up 的高速算力网络。[^cxl-4-whitepaper]
