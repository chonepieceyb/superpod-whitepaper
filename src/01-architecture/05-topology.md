...
NVSwtich是NVIDIA推出的实现节点内GPU之间高速互联的芯片，属于交换芯片（Switch ASIC）中的GPU专用交换芯片。它能够在单机或机柜级域内构建 GPU 全互连（all-to-all / non-blocking）通信网络，使网络直径达到 1，从而每个节点都可以直接与任意其他节点通信，无需经过中间交换节点。该架构特别适用于 32、72 或 144 节点的 Scale-Up 系统，可最大化节点间带宽利用率，显著降低通信延迟，并有利于大规模并行计算任务的高效调度与执行。【事实】
